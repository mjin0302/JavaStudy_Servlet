...
fetch("../empListJson")
  ...
  .then((result) => {
    ...
    result.forEach(function (item, idx, array) {
      // result : json(객체)들을 모아놓은 배열에서 항목하나당 item에 하나씩 가져옴
...
document
  ...
  .addEventListener("submit", addMemberFnc); // form에 submit 이벤트가 발생하면 실행하는 이벤트

// 전체선택 체크박스
document
  .querySelector("thead input[type='checkbox']")
  .addEventListener("change", allCheckBoxChange); // input태그(checkbox)에 변화가 나타나면 실행하는 이벤트

// 선택삭제 버튼
document
  .querySelector("#delSelectedBtn")
  .addEventListener("click", deleteCheckedFunc); // id가 delSelectedBtn인 태그를 클릭하면 발생하는 이벤트

// 데이터 한건 활용해서 tr요소를 생성
function makeTr(item) {
  // item은 json형식(객체)의 데이터 여야함
  let tr = document.createElement("tr");
  let titles = ["id", "lastName", "email", "hireDate", "job"]; // json(객체)의 value값을 가져올수있는 key역할을함

  titles.forEach(function (title) {
    // id => lstName => email ..... 등등 순서대로 가져옴
...
// fetch API => 비동기 방식 처리 => 동기식 처리(async, await)
// html: id값을 선택 삭제처리
